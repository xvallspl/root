...
#ifndef ROOT_TThreadExecutorImpl
#define ROOT_TThreadExecutorImpl
...
namespace ROOT {
   ...
   class TThreadExecutorImpl: public TExecutor<TThreadExecutorImpl> {
   public:
      explicit TThreadExecutorImpl();

      explicit TThreadExecutorImpl(UInt_t nThreads);

      TThreadExecutorImpl(TThreadExecutorImpl &) = delete;
      TThreadExecutorImpl &operator=(TThreadExecutorImpl &) = delete;
      ...
      using TExecutor<TThreadExecutorImpl>::Map;
      ...
      // // a static_assert check in TThreadExecutorImpl::Reduce is used to check that redfunc is compatible with the type returned by func
      using TExecutor<TThreadExecutorImpl>::MapReduce;
      ...
      using TExecutor<TThreadExecutorImpl>::Reduce;
   ...
   template<class F>
   void TThreadExecutorImpl::Foreach(F func, unsigned nTimes) {
   ...
   template<class F, class INTEGER>
   void TThreadExecutorImpl::Foreach(F func, ROOT::TSeq<INTEGER> args) {
   ...
   template<class F, class T>
   void TThreadExecutorImpl::Foreach(F func, std::initializer_list<T> args) {
   ...
   template<class F, class T>
   void TThreadExecutorImpl::Foreach(F func, std::vector<T> &args) {
   ...
   auto TThreadExecutorImpl::Map(F func, unsigned nTimes) -> std::vector<typename std::result_of<F()>::type> {
   ...
   auto TThreadExecutorImpl::Map(F func, ROOT::TSeq<INTEGER> args) -> std::vector<typename std::result_of<F(INTEGER)>::type> {
   ...
   auto TThreadExecutorImpl::Map(F func, unsigned nTimes, R redfunc, unsigned nChunks) -> std::vector<typename std::result_of<F()>::type> {
      if (nChunks == 0)
      {
   ...
   auto TThreadExecutorImpl::Map(F func, std::vector<T> &args) -> std::vector<typename std::result_of<F(T)>::type> {
   ...
   template<class F, class T, class Cond>
   auto TThreadExecutorImpl::Map(F func, std::array_view<T> &args) -> std::vector<typename std::result_of<F(T)>::type> {
      // //check whether func is callable
      using retType = decltype(func(args.front()));
   ...
   auto TThreadExecutorImpl::Map(F func, ROOT::TSeq<INTEGER> args, R redfunc, unsigned nChunks) -> std::vector<typename std::result_of<F(INTEGER)>::type> {
      if (nChunks == 0)
      {
...
/// \cond
   ...
   auto TThreadExecutorImpl::Map(F func, std::vector<T> &args, R redfunc, unsigned nChunks) -> std::vector<typename std::result_of<F(T)>::type> {
      if (nChunks == 0)
      {
         return Map(func, args);
      }
 ...
 template<class F, class T, class R, class Cond>
 auto TThreadExecutorImpl::Map(F func, std::array_view<T> &args, R redfunc, unsigned nChunks) -> std::vector<typename std::result_of<F(T)>::type> {
      if (nChunks == 0)
      {
         return Map(func, args);
      }
   ...
   /// A vector containg partial reductions' results is returned.
   template<class F, class T, class R, class Cond>
   auto TThreadExecutorImpl::Map(F func, std::initializer_list<T> args, R redfunc, unsigned nChunks) -> std::vector<typename std::result_of<F(T)>::type> {
      std::vector<T> vargs(std::move(args));
...
/// \endcond
   ...
   template<class F, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, unsigned nTimes, R redfunc) -> typename std::result_of<F()>::type {
      return Reduce(Map(func, nTimes), redfunc);
   }

   template<class F, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, unsigned nTimes, R redfunc, unsigned nChunks) -> typename std::result_of<F()>::type {
      return Reduce(Map(func, nTimes, redfunc, nChunks), redfunc);
   }

   template<class F, class INTEGER, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, ROOT::TSeq<INTEGER> args, R redfunc, unsigned nChunks) -> typename std::result_of<F(INTEGER)>::type {
      return Reduce(Map(func, args, redfunc, nChunks), redfunc);
   }
   /// \cond
   template<class F, class T, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, std::initializer_list<T> args, R redfunc, unsigned nChunks) -> typename std::result_of<F(T)>::type {
      return Reduce(Map(func, args, redfunc, nChunks), redfunc);
   }
   /// \endcond

   template<class F, class T, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, std::vector<T> &args, R redfunc) -> typename std::result_of<F(T)>::type {
      return Reduce(Map(func, args), redfunc);
   }

   template<class F, class T, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, std::vector<T> &args, R redfunc, unsigned nChunks) -> typename std::result_of<F(T)>::type {
      return Reduce(Map(func, args, redfunc, nChunks), redfunc);
   }

   template<class F, class T, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, std::array_view<T> &args, R redfunc) -> typename std::result_of<F(T)>::type {
      return Reduce(Map(func, args), redfunc);
   }

   template<class F, class T, class R, class Cond>
   auto TThreadExecutorImpl::MapReduce(F func, std::array_view<T> &args, R redfunc, unsigned nChunks) -> typename std::result_of<F(T)>::type {
      return Reduce(Map(func, args, redfunc, nChunks), redfunc);
   }
   ...
   auto TThreadExecutorImpl::Reduce(const std::vector<T> &objs, BINARYOP redfunc) -> decltype(redfunc(objs.front(), objs.front()))
   {
   ...
   template<class T, class R>
   auto TThreadExecutorImpl::Reduce(const std::vector<T> &objs, R redfunc) -> decltype(redfunc(objs))
   {
      // check we can apply reduce to objs
   ...
   template<class T, class R>
   auto TThreadExecutorImpl::SeqReduce(const std::vector<T> &objs, R redfunc) -> decltype(redfunc(objs))
   {
      return redfunc(objs);
   }
